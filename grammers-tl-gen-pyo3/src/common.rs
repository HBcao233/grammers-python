...
fn write_stubtype_pytlrequest<W: Write>(
  file: &mut W,
) -> io::Result<()> {
  writeln!(
    file, 
...
fn write_from_pytlrequest<W: Write>(
  file: &mut W,
  definitions: &[Definition],
) -> io::Result<()> {
  writeln!(file, r#"impl pyo3::FromPyObject<'_, '_> for PyTLRequest {{
  ...
  writeln!(file, r#"        Err(pyo3::PyErr::new::<pyo3::exceptions::PyTypeError, _>(
...
fn write_into_pytlrequest<W: Write>(
  file: &mut W,
  definitions: &[Definition],
) -> io::Result<()> {
  writeln!(file, r#"impl<'py> pyo3::IntoPyObject<'py> for PyTLRequest {{
...
fn write_stubtype_pytlobject<W: Write>(
  file: &mut W,
) -> io::Result<()> {
  writeln!(
    file, 
...
fn write_from_pytlobject<W: Write>(
  file: &mut W,
  definitions: &[Definition],
) -> io::Result<()> {
  writeln!(file, r#"impl pyo3::FromPyObject<'_, '_> for PyTLObject {{
    type Error = pyo3::PyErr;
    fn extract(obj: pyo3::Borrowed<'_, '_, pyo3::PyAny>) -> Result<Self, pyo3::PyErr> {{
        use pyo3::types::{{PyAnyMethods, PyTypeMethods}};"#)?;
  for def in definitions {
...
fn write_into_pytlobject<W: Write>(
  file: &mut W,
  definitions: &[Definition],
) -> io::Result<()> {
  writeln!(file, r#"impl<'py> pyo3::IntoPyObject<'py> for PyTLObject {{
    type Target = pyo3::PyAny;
    type Output = pyo3::Bound<'py, pyo3::PyAny>;
    type Error = pyo3::PyErr;
    fn into_pyobject(self, py: pyo3::Python<'py>) -> Result<pyo3::Bound<'py, pyo3::PyAny>, pyo3::PyErr> {{
...
fn write_serialize<W: Write>(
  ...
  writeln!(file, "impl grammers_tl_types::Serializable for PyTLObject {{")?;
  writeln!(file, "    fn serialize(&self, buf: &mut impl Extend<u8>) {{")?;
  writeln!(file, "        match self {{")?;
  for def in definitions {
    if def.category == Category::Types && !ignore_type(&def.ty) {
