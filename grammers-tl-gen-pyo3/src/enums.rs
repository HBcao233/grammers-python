...
fn write_stubtype<W: Write>(
  ...
  let type_hint = metadata.defs_with_type(ty).iter()
    .map(|d| {
...
fn write_serialize<W: Write>(
  ...
  writeln!(file, "{indent}    fn serialize(&self, buf: &mut impl Extend<u8>) {{")?;
...
fn write_deserialize<W: Write>(
  ...
  writeln!(file, "{indent}    fn deserialize(buf: crate::Buffer) -> grammers_tl_types::deserialize::Result<Self> {{")?;
  writeln!(file, "{indent}        use grammers_tl_types::Identifiable;")?;
  ...
  writeln!(file, "{indent}            _ => return Err(grammers_tl_types::deserialize::Error::UnexpectedConstructor {{ id }}),")?;
  writeln!(file, "{indent}        }})")?;
  writeln!(file, "{indent}    }}")?;
  writeln!(file, "{indent}}}")?;
