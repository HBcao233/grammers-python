...
.. warning::
    The content of this chapter is all TODO
...
.. code-block:: python
    ...
    from grammers import Client, events
...
<grammers.events.NewMessage>` *event*,
...
If a `NewMessage
<grammers.events.NewMessage>` event occurs,
and ``'hello'`` is in the text of the message, we `reply()
<grammers.custom.Message.reply>` to the event
with a ``'hi!'`` message.

.. note::

    Event handlers **must** be ``async def``. After all,
    grammers-python is an asynchronous library based on `asyncio`,
