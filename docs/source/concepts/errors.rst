...
You should import the errors from ``grammers.errors`` like so:
...
.. code-block:: python
    ...
    try:
        ...
        print('Must wait', e.value, 'before takeout')
...
All the "base" errors are listed in :ref:`grammers-errors`.
...
.. code-block:: text
    ...
    grammers.errors.rpcbaseerrors.BadRequestError: (400 MESSAGE_POLL_CLOSED, caused by SendVote, with value: )
...
-  ``FloodWaitError`` (420), the same request was repeated many times.
   Must wait ``.value`` (you can access this attribute). For example:
   ...
   .. code-block:: python
       ...
       try:
           ...
           print('Have to sleep', e.value, 'seconds')
           time.sleep(e.value)
...
You can refer to all errors from Python through the ``grammers.errors``
...
error instance, you can `.value` get the value.
...
.. code-block:: python
    ...
    from grammers.errors import FloodWaitError
    try:
        ...
    except FloodWaitError as e:
        print('Flood waited for', e.value)
        quit(1)
