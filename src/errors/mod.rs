...
use pyo3::exceptions::{PyException, PyTypeError, PyIOError};
...
impl PyRpcError {
  ...
  fn __str__(&self) -> PyResult<String> {
    ...
    Ok(format!("RpcError {}: {}{}", self.code, self.message, caused_by))
  }
...
impl From<InvocationErrorConverter> for PyErr {
  fn from(err: InvocationErrorConverter) -> PyErr {
    match err.0 {
      InvocationError::Rpc(e) => PyRpcError::new_err(
        e.code, e.name, e.value, e.caused_by, "".to_string(),
      ),
      InvocationError::Io(e) => PyIOError::new_err(
        format!("{}", e)
      ),
      InvocationError::Deserialize(e) => PyTypeError::new_err(
        format!("Deserialize: {}", e)
      ),
      InvocationError::Transport(e) => PyTypeError::new_err(
        format!("Transport: {}", e)
      ),
      ...
      InvocationError::Authentication(e) => PyTypeError::new_err(format!("Authentication: {}", e)),
    }
  }
}
