...
impl<'a, 'py> FromPyObject<'a, 'py> for ApiId {
    ...
    fn extract(ob: Borrowed<'a, 'py, PyAny>) -> Result<Self, Self::Error> {
        ...
        Err(PyErr::new::<PyTypeError, _>("api_id must be int or str"))
    }
}
...
impl PyClient {
    ...
    *,
    ...
    pub fn new<'py>(
        ...
        let session = Arc::new(
            SqliteSession::open(name.to_owned() + ".session")
                .map_err(|e: rusqlite::Error| PyRuntimeError::new_err(e.to_string()))?,
        );
        let pool = SenderPool::with_configuration(Arc::clone(&session), api_id.0, config);
