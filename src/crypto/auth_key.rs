...
impl PyAuthKey {
            ...
            BytesOrStr::Bytes(b) => {
                let data: [u8; 256] = (*b).try_into().map_err(|_| {
                    PyValueError::new_err(format!(
                        "AuthKey data must be exactly 256 bytes, got {}",
                        b.len()
                    ))
                })?;
    ...
    fn from_hex(hex_str: &str) -> PyResult<Self> {
        let data = grammers_crypto::hex::opt_from_hex(hex_str)
            .ok_or_else(|| PyValueError::new_err(format!("Invalid hex string: {}", hex_str)))?;
