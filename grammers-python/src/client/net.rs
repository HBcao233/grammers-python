...
use grammers_tl_types::{RemoteCall, Deserializable};
...
impl PyClient {
    pub async fn invoke(&self, request: PyTLRequest) -> PyResult<PyTLObject> {
        self.invoke_raw(request).await
            .map_err(|e| {
                InvocationErrorConverter(e).into()
            })
    }
...
impl PyClient {
    pub async fn invoke_raw_in_dc(&self, dc_id: i32, request: PyTLRequest) -> Result<PyTLObject, InvocationError> {
        use grammers_tl_types::{Deserializable, Serializable};
        let response = self.do_invoke_in_dc(dc_id, request.to_bytes()).await?;
        PyTLObject::from_bytes(&response)
            .map_err(Into::into)
    }
    ...
    pub async fn invoke_raw(&self, request: PyTLRequest) -> Result<PyTLObject, InvocationError> {
        ...
        let dc_id = session.home_dc_id().await.map_err(|e| InvocationError::PyErr(e))?;
        self.invoke_raw_in_dc(dc_id, request).await
    }
    ...
    pub async fn invoke_tl<R: RemoteCall>(&self, request: &R) -> Result<R::Return, InvocationError> {
