...
impl Normal {
    ...
    pub fn inviter_id(&self) -> Option<PyPeerId> {
        self.inviter_id.map(|p| PyPeerId::user(p).unwrap())
    }
}
...
impl Admin {
    ...
    pub fn promoted_by(&self) -> Option<PyPeerId> {
        self.promoted_by.map(|p| PyPeerId::user(p).unwrap())
    }
...
impl Banned {
    ...
    pub fn kicked_by(&self) -> PyPeerId {
        PyPeerId::user(self.kicked_by).unwrap()
    }
...
impl Participant {
    pub(crate) fn from_raw_channel(
        ...
        match participant {
            ...
            P::Banned(p) => Self {
                user: match peers.take(PyPeerId::from(p.peer.clone())).unwrap() {
                    Peer::User(user) => user,
