...
impl Group {
    ...
    pub fn id(&self) -> PyPeerId {
        ...
        match &self.raw {
            Chat::Empty(chat) => PyPeerId::chat(chat.id).unwrap(),
            Chat::Chat(chat) => PyPeerId::chat(chat.id).unwrap(),
            Chat::Forbidden(chat) => PyPeerId::chat(chat.id).unwrap(),
            Chat::Channel(channel) => PyPeerId::channel(channel.id).unwrap(),
            Chat::ChannelForbidden(channel) => PyPeerId::channel(channel.id).unwrap(),
        }
    }
    ...
    pub(crate) fn auth(&self) -> Option<PyPeerAuth> {
        ...
        Some(match &self.raw {
            Chat::Empty(_) => PyPeerAuth::default(),
            Chat::Chat(_) => PyPeerAuth::default(),
            Chat::Forbidden(_) => PyPeerAuth::default(),
...
impl From<Group> for PyPeerInfo {
    #[inline]
    fn from(group: Group) -> Self {
        <Self as From<&Group>>::from(&group)
    }
}
impl<'a> From<&'a Group> for PyPeerInfo {
